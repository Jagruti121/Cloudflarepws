 ...
 *
 * Props:
 *   session        — the session object from groupedSessions (includes metadata + is_active)
 *   students       — sorted, filtered array of student objects for this session
 *   search         — current search term (controlled by parent)
 *   onSearchChange — (value: string) => void
 */
const InternalSessionTable = ({ session, students, search, onSearchChange }) => {
    ...
    try {
      const rows = students.map((sub) => {
        ...
        return {
          ...
          'Total Marks': parseFloat(sub.scores?.total) || parseFloat(sub.scores?.internal) || 0,
        };
      });

      const worksheet = XLSX.utils.json_to_sheet(rows);
      const workbook = XLSX.utils.book_new();
      XLSX.utils.book_append_sheet(workbook, worksheet, 'Internal Result');
      ...
      XLSX.writeFile(workbook, 'result of internal exam.xlsx');
    } catch (err) {
  ...
  return (
    <div>
      {/* ── Session Header Description Block ──────────────────────────── */}
      <div className="mb-6 p-5 rounded-2xl border border-purple-100 bg-gradient-to-br from-purple-50 to-indigo-50 shadow-sm">
        <div className="flex items-center gap-2 mb-4">
          <span className="bg-purple-600 text-white text-xs font-bold px-3 py-1 rounded-full uppercase tracking-wide">
            Internal Exam
          </span>
          <span className="text-sm text-purple-700 font-semibold">{session?.session_code}</span>
        </div>
        <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-5 gap-4">
          <div>
            <p className="text-xs font-semibold text-purple-400 uppercase tracking-wider mb-1">Date Conducted</p>
            <p className="text-sm font-semibold text-gray-800">{dateConducted}</p>
          </div>
          <div>
            <p className="text-xs font-semibold text-purple-400 uppercase tracking-wider mb-1">Department</p>
            <p className="text-sm font-semibold text-gray-800">{department}</p>
          </div>
          <div>
            <p className="text-xs font-semibold text-purple-400 uppercase tracking-wider mb-1">Subject</p>
            <p className="text-sm font-semibold text-gray-800">{subjectName}</p>
          </div>
          <div>
            <p className="text-xs font-semibold text-purple-400 uppercase tracking-wider mb-1">Semester</p>
            <p className="text-sm font-semibold text-gray-800">{formatSemester(semester)}</p>
          </div>
          <div>
            <p className="text-xs font-semibold text-purple-400 uppercase tracking-wider mb-1">Created By</p>
      ...
      <div className="flex flex-col sm:flex-row gap-3 justify-between items-start sm:items-center mb-5">
        <div className="relative w-full sm:w-72">
          ...
          <input
            ...
            className="w-full pl-9 pr-4 py-2 border border-gray-200 rounded-lg focus:ring-2 focus:ring-purple-500 focus:border-purple-500 focus:outline-none text-sm transition"
          />
        </div>
        ...
        <button
          ...
          className="flex items-center gap-2 bg-purple-600 hover:bg-purple-700 disabled:opacity-50 disabled:cursor-not-allowed text-white px-5 py-2 rounded-lg font-semibold text-sm transition shadow-sm whitespace-nowrap"
        >
          {exportLoading ? (
        ...
        <table className="w-full text-sm">
          <thead>
            <tr className="bg-gradient-to-r from-purple-50 to-indigo-50 border-b border-gray-200">
              <th className="text-left px-4 py-3 font-semibold text-gray-600 whitespace-nowrap">S.No.</th>
              ...
              <th className="text-center px-4 py-3 font-semibold text-gray-600 whitespace-nowrap">Total Marks</th>
            </tr>
          </thead>
          <tbody>
            {students.length === 0 ? (
              ...
              students.map((sub, idx) => {
                const totalMarks = parseFloat(sub.scores?.total) || parseFloat(sub.scores?.internal) || 0;
                const isAbsent = sub.status === 'absent' || sub.status === 'registered';
                const statusLabel = isAbsent ? 'Absent' : 'Present';
                return (
                  <tr key={sub.id || sub.roll_no} className="border-b border-gray-50 hover:bg-purple-50/30 transition-colors">
...
export default InternalSessionTable;
